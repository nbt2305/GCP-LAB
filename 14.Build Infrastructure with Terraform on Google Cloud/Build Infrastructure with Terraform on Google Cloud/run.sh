...
# Input BUCKET
if [[ -z "$BUCKET" ]]; then
    read -p "Input BUCKET: " BUCKET
fi

# Input INSTANCE
if [[ -z "$INSTANCE" ]]; then
    read -p "Input INSTANCE: " INSTANCE
fi

# Input VPC
if [[ -z "$VPC" ]]; then
    read -p "Input VPC: " ZONE
fi

# Export env
export INPUT_ERROR="${RED}Errors: Please setup BUCKET, INSTANCE, ZONE env before run script.${RESET}"

# Check env
if [[ -z "$BUCKET" || -z "$INSTANCE" || -z "$ZONE" ]]; then
    echo -e $INPUT_ERROR
    exit 1
fi

export BUCKET INSTANCE ZONE
